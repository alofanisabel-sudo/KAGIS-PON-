...
export default function Stock() {
  ...
  async function enregistrer() {
    ...
    try {
      ...
      await addDoc(collection(db, "logs"), {
        ...
        cible: `Produit "${nomP.trim()}" (${Number(quantite) || 0} en stock, ${fmt(Number(prix))})`,
        auteurId: user.uid,
  ...
  async function confirmerSuppression(id: string) {
    ...
    if (p && user) {
      await addDoc(collection(db, "logs"), {
        ...
        cible: `Produit "${p.nom}" (${p.quantite} en stock, ${fmt(p.prix)})`,
        auteurId: user.uid,
        auteurNom: nom || user.email,
        date: serverTimestamp(),
      });
  ...
  async function confirmerVente() {
    ...
    try {
      ...
      batch.set(logRef, {
        action: "vente",
        cible: `Vente de ${qte}x "${venteProduit.nom}" (${fmt(montantTotal)})`,
        auteurId: user.uid,
        auteurNom: nom || user.email,
        date: serverTimestamp(),
      });
