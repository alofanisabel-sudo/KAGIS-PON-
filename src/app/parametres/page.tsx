...
import { ChevronRight, User, Users, Shield, LogOut, X, Sun, Moon, FileClock, UserX, UserCheck } from "lucide-react";
...
export default function Parametres() {
  ...
  async function toggleActif(membreId: string, actifActuel: boolean) {
    await updateDoc(doc(db, "users", membreId), { actif: !actifActuel });
  }
            ...
            >
              ...
              <div className="flex flex-col gap-2">
                {membres.map((m) => {
                  ...
                  const estSoiMeme = m.id === user.uid;
                  return (
                    <div
                      key={m.id}
                      className="flex items-center gap-3 bg-black border border-border rounded-xl px-3 py-2.5"
                      style={{ opacity: estActif ? 1 : 0.5 }}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {!estActif && (
                          <div className="font-mono text-[9px] text-negative tracking-wide mt-0.5">DÉSACTIVÉ</div>
                        )}
                      </div>
                      <span
                      ...
                      {role === "admin" && !estSoiMeme && (
                        <button
                          onClick={() => toggleActif(m.id, estActif)}
                          className="flex-shrink-0"
                          style={{ color: estActif ? "#B5533C" : "#6FA287" }}
                          aria-label={estActif ? "Désactiver" : "Réactiver"}
                        >
                          {estActif ? <UserX size={16} /> : <UserCheck size={16} />}
                        </button>
                      )}
                    </div>
                  );
